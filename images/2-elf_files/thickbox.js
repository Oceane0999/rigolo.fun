...
var tb_pathToImage = "/images/loadingAnim.gif";
	...
	try {
	   ...
	   var urlType = baseURL.toLowerCase().match(urlString);
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
			...
			$J("#TB_window").append("<a href='' id='TB_ImageOff' title='Close'><img id='TB_Image' src='/"+url+"' width='"+imageWidth+"' height='"+imageHeight+"' alt='"+caption+"'/></a>" + "<div id='TB_caption'>"+caption+"<div id='TB_secondLine'>" + TB_imageCount + TB_PrevHTML + TB_NextHTML + "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='Close'>Fermer</a> ou pressez Esc</div>"); 		
			...
			var maxDim = tb_getPageSize();


			//if((maxDim[0]-100) > ((params['width']*1)+30)) {TB_WIDTH = (params['width']*1) + 30;} else {TB_WIDTH = (maxDim[0]*1)-100;}
			//if((maxDim[1]-100) > ((params['height']*1)+40)) {TB_HEIGHT = (params['height']*1) + 40;} else {TB_HEIGHT = (maxDim[1]*1)-100;}
			if(navigator.appName=="Microsoft Internet Explorer") {
			if((maxDim[0]-100) > ((params['width']*1)+30) && (maxDim[1]-100) > ((params['height']*1)+40)) {
			  TB_WIDTH = (params['width']*1) + 30;
			  TB_HEIGHT = (params['height']*1) + 40;
			}
			else {
			  TB_WIDTH = (maxDim[0]*1)-100;
			  TB_HEIGHT = (maxDim[1]*1)-100;
			}}
			else {
			  if((maxDim[0]-100) > ((params['width']*1)+35)) {TB_WIDTH = (params['width']*1) + 35;} else {TB_WIDTH = (maxDim[0]*1)-100;}
			  if((maxDim[1]-100) > ((params['height']*1)+45)) {TB_HEIGHT = (params['height']*1) + 45;} else {TB_HEIGHT = (maxDim[1]*1)-100;}
			}

			//TB_WIDTH = (params['width']*1) + 30 || 630; //defaults t no paramaters were added to URL
			...
			if(url.indexOf('TB_iframe') != -1){// either iframe or ajax window		
					...
					if(params['modal'] != "true"){//iframe no modal
						$J("#TB_window").append("<div id='TB_title'><div id='TB_ajaxWindowTitle'>"+caption+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='Close'>Fermer</a> ou pressez Esc</div></div><iframe frameborder='0' hspace='0' src='/"+urlNoQuery[0]+"' id='TB_iframeContent' name='TB_iframeContent"+Math.round(Math.random()*1000)+"' onload='tb_showIframe()' style='width:"+(ajaxContentW + 29)+"px;height:"+(ajaxContentH + 17)+"px;' > </iframe>");
					}else{//iframe modal
					$J("#TB_overlay").unbind();
					...
					if($J("#TB_window").css("display") != "block"){
						...
						$J("#TB_window").append("<div id='TB_title'><div id='TB_ajaxWindowTitle'>"+caption+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'>Fermer</a> ou pressez Esc</div></div><div id='TB_ajaxContent' style='width:"+ajaxContentW+"px;height:"+ajaxContentH+"px'></div>");
						}else{//ajax modal
						$J("#TB_overlay").unbind();
